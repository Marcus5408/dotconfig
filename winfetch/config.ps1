...
function info_custom_pkgs {
    # $scoop = ((scoop list) -replace "Installed apps:\n" | Measure-Object | Select-Object -ExpandProperty Count)
    # scoop seems to print "Installed apps:" outside of `scoop list` so I can't remove it. hardcoded for now
    $scoop = 12
    
    # $choco = choco list --local-only | Measure-Object | Select-Object -ExpandProperty Count
    # slow. hardcoded lol
    $choco = 2

    # $winget = winget list | Measure-Object | Select-Object -ExpandProperty Count
    # winget is slow, for now its hardcoded
    $winget = 282
    
    # $pip = pip list | Measure-Object | Select-Object -ExpandProperty Count
    # slow, hardcoded
    $pip = 216
    return @{
        title = "Packages"
        content = "$scoop (scoop), $choco (choco), $winget (winget), $pip (pip)"
    }
}
